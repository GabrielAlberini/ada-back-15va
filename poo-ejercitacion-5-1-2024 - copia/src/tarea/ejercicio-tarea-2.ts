...
class Family {
  ...
  getOldestMember() {
    const membersAges = this.members.map((individual) => individual.age); // [6, 51, 55]
    let maxAge = Math.max(...membersAges);

    // for (let i = 0; i <= membersAges.length; i++) {
    //   if (maxAge < membersAges[i]) {
    //     maxAge = membersAges[i];
    //   }
    // }
...
(function testFamily() {
  ...
  console.log(oldestNumberMember);
